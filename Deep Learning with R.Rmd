...
![Single Layer Neural Network](Images/single layer NN.jpeg)
...
\newpage
\section{DL 11 The cross-entropy Loss Function}
It is easy for us to define the amount of error in numerical predictions, for example we can use the loss function to find the square of the difference between true and predicted values. However, for the majority of problems in real-life, we need to have categorical predictions. In the case of performing categorical predictions, how to we conduct error analysis?

\subsection{Entropy}
The the following example where we have the following set of words: **{car, car, car, car, car, car, bus, bus, bus, bus, human, stop sign}**. We can see that each word: car, bus, human, and stop sign, has a probability of occurring in the set: P{car,bus, human, stop sign} = ($\frac{1}{2}$, $\frac{1}{3}$, $\frac{1}{12}$, $\frac{1}{12}$). The entropy then can be calculated as follow:
$$1\times \frac{1}{2} + 2\times \frac{1}{3} + 3\times \frac{1}{12} + 3\times \frac{1}{12} \approx 1.667$$
where:
\begin{enumerate}
  \item 1 in the first term means the number of decisions we need to make to arrive at the conclusion that the word is car; $\frac{1}{2}$ is the probability of the word car appear in the set
  \item 2 in the second term means that it takes two decision nodes for the word bus to be selected, with a probability of appearing in the set to be $\frac{1}{3}$
\end{enumerate}

Thus, the entropy in information gain is:
$$E(d) = -\Sigma_{i=1}^k[p_ilog_2(p_i)]$$
where:
\begin{enumerate}
  \item d is the vector of element
  \item E is the entropy
  \item $p_i$ is the probability of each element
\end{enumerate}


\subsection{Cross entropy}
Different from entropy, cross entropy is comparing two distributions. Recall that in neural networks, we are inputting numerical matrices after using encoding on categorical attributes (either by one-hot encoding or multi-hot encoding) to the network and then the NN outputs numerical predictions (maybe representing the probability of classification). In the case of finding entropy of encoded numerical predictions, we can use cross entropy:
$$H(y, \hat{y}) = -\Sigma_{i = 1}^k[y_iln(\hat{y_i})]$$
where $y_i$ is the actual value, and $\hat{y_i}$ is the predicted value

\newpage
\section{DL 12 Deep neural networks for regression problems}
\subsection{Data \& preprocessing}
```{r, message = FALSE, warning = FALSE}
library(readr)
library(keras)
...
\newpage
\section{Convolutional Neural Network "CNN" in R}
A type of network architecture targeting for solving image-related problems
\subsection{Convolution operation}
The convolution operation is conducted in the manner shown below:
$$\begin{bmatrix} 
1 & 2 & 3 \\ 
4 & 3 & 3\\
3 & 4 & 2
...
7 & 2 & 2
\end{bmatrix}$$
the two matrices are multiplied element-wise to obtain a single numerical representation
$$(1\times3)+(2\times3)+(3\times2)+(4\times1)+\dots(4\times2)+(2\times2) = 61$$
\subsection{Image filtering}
\subsubsection{Filtering intuition}
In image processing, we usually resort to convolutional neural network that uses a filtering layer called the *kernel* or *filter*, usually with size $3\times3$, to filter through the entire image. Because the filtering process is followed by convolution operation as shown in the previous section, the size of the resulting matrix is usually smaller. If the starting image has size $n\times n$ and the kernel size is $m\times m$, then the resulting size is:
$$(n-m+1)\times(n-m+1)$$
Moreover, colorful images are not just a single layer of pixels, multiple layers are used to construct the variety of colors, thus, we shall use multiple kernels. In the case when p filters are used, the resultant image has the dimension:
$$(n-m+1)\times(n-m+1)\times p$$
\subsubsection{Padding}
From the last section, we can see that if we move the kernel across the image for filtering, the edges participate less in the convolution than pixels more towards the center. Thus, in order to increase the participation of edges in convolution operation, we use the technique of padding zero-valued pixels around the original image

\subsubsection{Stride}
Stride means the size of step that the kernel takes to filter across the image

\subsubsection{Pooling}
A reduction technique in CNN, either take the average or mode within a selected frame and conduct the same operation across the entire image

\subsubsection{Flattening}
Before outputting for the convolution network, we need to flatten out the result (i.e. transform the numerical matrices into numerical vectors), so it can be passed into a densely connected layer for further processing

\newpage
\section{Example of convolutional neural network}
\subsection{Data \& preprocessing}
```{r}
library(keras)
